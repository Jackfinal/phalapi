...
class Toolkit
{
     ...
     * @return array
     */
    public static function getPlayUrls(array $datas)
    {
        if (empty($datas)) {
            return array();
        }
        $host = Yii::app()->params['host'];
        $webPort = Yii::app()->params['webPort'];
        $file_server_path = Yii::app()->params['file_server_path'];
        $baseUrl = "http://{$host}:$webPort/";
        $urls = array();
        $stationModels = Stations::model()->findAll();
        $stations = array();
        foreach ($stationModels as $k => $v) {
            $stations[$v->station_number] = long2ip($v->ip);
        }
        foreach ($datas as $key => $val) {
            $id = $val['id'];
            $urls[$id] = '';
            if ($val['existed_file'] == 0) {
                //取工作站地址
                if (isset($stations[$val['station_id']])) {
                    $stationIp = $stations[$val['station_id']];
                    $urls[$id] = 'http://' . $stationIp . ":" . $webPort . "/" . $file_server_path . Toolkit::getInformationFilePath($val);
                }
            } elseif ($val['existed_file'] == 1) {
                //mysql版本,这里不考虑 独立ftp存储服务器
                //多盘存储的apache别名
                if (Yii::app()->params['is_multi_disk_storage'] && !empty($val['path']) && $val['path']!="D") {
                    $apache_ftp_path = $val['path'] . "/";
                } else {
                    $apache_ftp_path = "";
                }
                $urls[$id] =  $baseUrl . $apache_ftp_path . $file_server_path . "/" . $val['station_id'] . Toolkit::getInformationFilePath($val);
            }
        }
        return $urls;
    }
     ...
     * @return array
     */
    public static function getDownloadUrls(array $datas)
    {
        if (empty($datas)) {
            return array();
        }
        ...
        foreach ($datas as $key => $val) {
            ...
            if ($val['existed_file'] == 0) {
                ...
                if (isset($stations[$val['station_id']])) {
                    $stationIp = $stations[$val['station_id']];
                    $urls[$val['id']] = 'http://' . $stationIp . ":" . $webPort."/?r=resource/download&archive_num={$val['archive_num']}";
                }
            } elseif ($val['existed_file'] == 1) {
                //mysql版本,这里不考虑 独立ftp存储服务器
                $urls[$val['id']] = $baseUrl . "/?r=resource/download&archive_num={$val['archive_num']}";
            }
        }
        return $urls;
    }
